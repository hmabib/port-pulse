...
import { SEGMENT_ITEMS, type MainTabId, type SegmentId } from "./ui/Navigation";
...
function buildCumul2026MonthlyRows(
  ...
  return Array.from(dailyByMonth.entries())
    .map(([anneeMois, rows]) => {
      ...
      const activeDays = sorted.filter((r) => toNumber(r.gate_total_mouvements) > 0 || toNumber(r.ttt_total_camions) > 0);
      const nbJours = activeDays.length || 1;
      const allDays = sorted.length || 1;
      ...
      const nbGateDays = activeGate.length || 1;
      ...
      return {
        anneeMois,
        moisLabel,
        latestDate: normalizeDateValue(resolveRowDate(lastDay)),
        nbJours: gateMonth.length,
        // Cumuls from last day
        totalTeu: toNumber(lastDay.total_teu),
        importTeu: toNumber(lastDay.import_teu),
        exportTeu: toNumber(lastDay.export_teu),
        transboTeu: toNumber(lastDay.transbo_teu),
        videsTeu: toNumber(lastDay.vides_teu),
        totalForecast: toNumber(lastDay.total_forecast),
        tauxRealisation: toNumber(lastDay.taux_realisation_total_pct),
        // Averages across month
        occupationAvg: occSum / allDays,
        reefersAvg: reefSum / allDays,
        tttAvg: tttSum / nbGateDays,
        camionsAvgJour: camionsSum / nbGateDays,
        mouvementsAvgJour: mvtSum / nbGateDays,
        sortiesPleinAvgJour: spSum / nbGateDays,
        sortiesVideAvgJour: svSum / nbGateDays,
        sortiesTotalAvgJour: (spSum + svSum) / nbGateDays,
        entreesPleinAvgJour: epSum / nbGateDays,
        entreesVideAvgJour: evSum / nbGateDays,
...
function buildMonthlyBulletin(dailyRows: GenericRow[], gateRows: GenericRow[]): BulletinMonthRow[] {
  ...
  for (const row of dailyRows) {
    const monthKey = toText(row.annee_mois, "");
    if (!monthKey) continue;
    const dateText = toText(row.date_rapport, "");
  ...
  return Array.from(monthMap.entries())
    .map(([anneeMois, entry]) => {
      ...
      return {
        anneeMois,
        annee: toNumber(latestDaily.annee),
        moisLabel: toText(latestDaily.mois_nom_fr, anneeMois),
        ...
        latestDate: toText(latestDaily.date_rapport, ""),
      };
    })
    .sort((a, b) => a.anneeMois.localeCompare(b.anneeMois));
}
...
function DataBadge({ type }: { type: "jour" | "cumul-mois" | "moy-mois" | "cumul-annuel" | "moy-annuel" | "tendance" }) {
  const config: Record<string, { label: string; color: string }> = {
    "jour": { label: "Jour", color: "var(--cyan)" },
    "cumul-mois": { label: "Cumul mois", color: "var(--emerald)" },
    "moy-mois": { label: "Moy. mois", color: "var(--blue)" },
    "cumul-annuel": { label: "Cumul annuel", color: "var(--violet)" },
    "moy-annuel": { label: "Moy. annuel", color: "var(--amber)" },
    "tendance": { label: "Tendance", color: "var(--rose)" },
  };
...
export default function DashboardClient({
  ...
  const [year, setYear] = useState(() =>
    filterOptions.years.includes(2026) ? "2026" : String(filterOptions.years[0] ?? ""),
  );
  const [month, setMonth] = useState("");
  const [day, setDay] = useState("");
  const [shipping, setShipping] = useState("");
  const [activeTab, setActiveTab] = useState<MainTabId>("situation");
  const [activeSegment, setActiveSegment] = useState<SegmentId>("global");
      ...
      try {
        const [dailyRes, monthlyRes, perfRes, gateRes, armRes, expRes, attRes, appRes, opRes, escRes, parcRes, rapRes, kpiRes] =
          await Promise.all([
            ...
            fetch(`/api/data/kct_escales_armateurs?limit=40&orderBy=date_rapport&orderDir=DESC&${qs}`, { signal: controller.signal }).then((r) => r.json()),
            fetch(`/api/data/kct_exploitants_parc?limit=40&orderBy=date_rapport&orderDir=DESC&${qs}`, { signal: controller.signal }).then((r) => r.json()),
            fetch(`/api/data/kct_navires_attendus?limit=12&orderBy=date_rapport&orderDir=DESC&${qs}`, { signal: controller.signal }).then((r) => r.json()),
            ...
            fetch(`/api/data/kct_navires_operation?limit=20&orderBy=date_rapport&orderDir=DESC&${qs}`, { signal: controller.signal }).then((r) => r.json()),
            fetch(`/api/data/kct_operations_escales?limit=20&orderBy=date_rapport&orderDir=DESC&${qs}`, { signal: controller.signal }).then((r) => r.json()),
  ...
  const cumul2026Monthly = useMemo(
    () => buildCumul2026MonthlyRows(dashboardData.dailyData, dashboardData.gateData, dashboardData.naviresPerformance, selectedCumulYear),
    [dashboardData.dailyData, dashboardData.gateData, dashboardData.naviresPerformance, selectedCumulYear],
  );
  const cumul2026Shipping = useMemo(
    () => buildCumul2026ShippingRows(dashboardData.naviresPerformance, selectedCumulYear),
    [dashboardData.naviresPerformance, selectedCumulYear],
  );
  ...
  const cumul2026Annual = useMemo(() => {
    const n = cumul2026Monthly.length || 1;
    return {
      totalTeu: cumul2026Monthly.reduce((s, r) => s + r.totalTeu, 0),
      ...
      totalForecast: cumul2026Monthly.reduce((s, r) => s + r.totalForecast, 0),
      ...
      tauxRealisationLast: cumul2026Monthly.length ? cumul2026Monthly[cumul2026Monthly.length - 1].tauxRealisation : 0,
      occupationAvg: cumul2026Monthly.reduce((s, r) => s + r.occupationAvg, 0) / n,
      reefersAvg: cumul2026Monthly.reduce((s, r) => s + r.reefersAvg, 0) / n,
      tttAvg: cumul2026Monthly.reduce((s, r) => s + r.tttAvg, 0) / n,
      camionsAvgJour: cumul2026Monthly.reduce((s, r) => s + r.camionsAvgJour, 0) / n,
      sortiesTotalAvgJour: cumul2026Monthly.reduce((s, r) => s + r.sortiesTotalAvgJour, 0) / n,
      sortiesPleinAvgJour: cumul2026Monthly.reduce((s, r) => s + r.sortiesPleinAvgJour, 0) / n,
      sortiesVideAvgJour: cumul2026Monthly.reduce((s, r) => s + r.sortiesVideAvgJour, 0) / n,
      productivityAverage: cumul2026Monthly.reduce((s, r) => s + r.productivityAverage, 0) / n,
    };
  }, [cumul2026Monthly]);
  ...
  const availableDates = useMemo(() => {
    ...
    for (const row of dashboardData.dailyData) {
  ...
  const situationDateRows = useMemo(() => filterRowsByDate(dashboardData.dailyData, selectedSituationDate), [dashboardData.dailyData, selectedSituationDate]);
  const situationDateGateRows = useMemo(() => filterRowsByDate(dashboardData.gateData, selectedSituationDate), [dashboardData.gateData, selectedSituationDate]);
  const situationDateParcRows = useMemo(() => filterRowsByDate(dashboardData.parcConteneurs, selectedSituationDate), [dashboardData.parcConteneurs, selectedSituationDate]);
  const situationDateKpiRows = useMemo(() => filterRowsByDate(dashboardData.kpisData, selectedSituationDate), [dashboardData.kpisData, selectedSituationDate]);
  const situationDatePerformanceRows = useMemo(() => filterRowsByDate(dashboardData.naviresPerformance, selectedSituationDate), [dashboardData.naviresPerformance, selectedSituationDate]);
  const situationDateAttendusRows = useMemo(() => filterRowsByDate(dashboardData.naviresAttendus, selectedSituationDate), [dashboardData.naviresAttendus, selectedSituationDate]);
  const situationDateAppareillesRows = useMemo(() => filterRowsByDate(dashboardData.naviresAppareilles, selectedSituationDate), [dashboardData.naviresAppareilles, selectedSituationDate]);
  const situationDateOperationRows = useMemo(() => filterRowsByDate(dashboardData.naviresOperation, selectedSituationDate), [dashboardData.naviresOperation, selectedSituationDate]);
  ...
  const occupancyTrend = useMemo(() => dashboardData.parcConteneurs.slice(-21), [dashboardData.parcConteneurs]);
  const gateFlowTrend = useMemo(() => dashboardData.gateData.slice(-14), [dashboardData.gateData]);
  const gateTttTrend = useMemo(() => dashboardData.gateData.slice(-21), [dashboardData.gateData]);
    ...
    <div className="min-h-screen text-[var(--text-primary)]">
      ...
      <div className="mx-auto flex min-h-screen max-w-[1920px] gap-5 p-4 lg:p-5">
        ...
        <Navigation
          ...
          isLoading={isLoading}
          latestDate={formatDateLabel(latestDashboardDate)}
        ...
        <main className="min-w-0 flex-1 space-y-5">
          ...
          <header className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-bg)] p-5 backdrop-blur-sm theme-transition">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="text-2xl font-bold tracking-tight text-[var(--text-primary)]">
                    ...
                    {activeTab === "operations" && "Synthese operationnelle"}
                ...
                <div className="mt-1.5 flex items-center gap-3">
                  <p className="text-2xl font-black tracking-tight text-[var(--cyan)]">
                    {activeTab === "situation" ? formatDateLabel(selectedSituationDate) : formatDateLabel(latestDashboardDate)}
                  </p>
                  <p className="text-[13px] text-[var(--text-muted)]">
                    {activeTab === "situation" && <><DataBadge type="jour" /> Donnees a la date du bulletin</>}
                    ...
                    {activeTab === "operations" && <><DataBadge type="tendance" /> Synthese operationnelle</>}
                    {(activeTab === "segments" || activeTab === "navires" || activeTab === "croisee" || activeTab === "intelligence") && <>Cockpit d&apos;exploitation terminal</>}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const chartImages = await captureCharts(".recharts-responsive-container");
                    ...
                    const sections: PdfSection[] = [
                      { title: "Volumes TEU", rows: dashboardData.dailyData.slice(-15).reverse(), columns: volColumns, chartImage: chartImages[0] },
                      { title: "Gate / TTT", rows: dashboardData.gateData.slice(-15).reverse(), columns: gateColumns, chartImage: chartImages[1] },
                      { title: "Parc conteneurs", rows: dashboardData.parcConteneurs.slice(-15).reverse(), columns: parcColumns, chartImage: chartImages[2] },
                      { title: "Escales armateurs", rows: dashboardData.armateursData.slice(-15).reverse(), columns: escColumns, chartImage: chartImages[3] },
                    ];
            ...
            <div className="mt-4 flex flex-wrap gap-1.5 lg:hidden">
              {(["situation", "cumul2026", "operations", "bulletin", "segments", "navires", "analyse", "croisee", "intelligence"] as MainTabId[]).map((tab) => {
                const labels: Record<string, string> = { situation: "Situation", cumul2026: "Cumul 2026", operations: "Synthese", bulletin: "Bulletin", segments: "Segments", navires: "Navires", analyse: "Analyse", croisee: "Croisee", intelligence: "Intel." };
                return (
                  <button
                    key={tab}
                    type="button"
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard
              ...
              tone="#10b981"
              hint={`Bulletin ${formatDateLabel(selectedSituationDate)} | Budget ${formatInteger(situationDaily.total_forecast)} TEU`}
            ...
            <MetricCard
              ...
              tone="#f59e0b"
              hint={`Jour ${formatDateLabel(selectedSituationDate)} | ${formatInteger(situationGate.gate_total_mouvements)} mvts | TTT ${formatMinutes(situationGate.ttt_duree_minutes)}`}
            ...
            <MetricCard
              ...
              tone="#8b5cf6"
              hint={`Jour ${formatDateLabel(selectedSituationDate)} | ${formatInteger(situationParc.parc_conteneurs_utilise)} / ${formatInteger(situationParc.parc_conteneurs_total)} EVP`}
          ...
          {activeTab === "situation" && (
            <>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
                <MetricCard label="Date de reference" value={formatDateLabel(latestDashboardDate)} tone="#0f766e" hint={toText(situationDaily.jour_nom_fr, "Derniere data")} icon={<CalendarRange className="h-4 w-4" />} compact />
                ...
                <MetricCard label="Navires" value={formatInteger(situationOperation.length || situationDaily.nb_navires_en_operation)} tone="#06b6d4" hint={`${formatInteger(situationAttendus.length || situationDaily.nb_navires_attendus)} att. | ${formatInteger(situationAppareilles.length || situationDaily.nb_navires_appareilles)} app.`} icon={<Ship className="h-4 w-4" />} compact />
              </div>

              <div className="grid gap-5 xl:grid-cols-2">
                <SectionCard title="Occupation parc detaillee" subtitle={<><DataBadge type="tendance" /> Utilise, disponible et variation journaliere sur 21 jours</>}>
                  ...
                  <div className="h-[340px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={occupancyTrend}>
                        ...
                        <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} valueFormatter={(v, name) => String(name).includes("Tx") ? formatPercent(v) : formatInteger(v)} />} />
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Line type="monotone" dataKey="parc_conteneurs_utilise" name="Utilise" stroke="#10b981" strokeWidth={2.5} dot={{ r: 2 }} />
                        <Line type="monotone" dataKey="parc_conteneurs_disponible" name="Disponible" stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 2 }} />
                        <Line type="monotone" dataKey="taux_occupation_parc" name="Tx parc %" stroke="#8b5cf6" strokeWidth={2.2} dot={{ r: 2 }} />
                        <Line type="monotone" dataKey="taux_occupation_reefers" name="Tx reefer %" stroke="#f59e0b" strokeWidth={2.2} dot={{ r: 2 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>

                <SectionCard title="Pleins, vides et mouvements" subtitle={<><DataBadge type="tendance" /> Evolution recente des flux gate | {formatInteger(situationGate.gate_total_mouvements)} mvts | TTT {formatMinutes(situationGate.ttt_duree_minutes)}</>}>
                  <div className="h-[400px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={gateFlowTrend}>
                        ...
                        <Bar dataKey="gate_entrees_vides" name="Vides entrants" fill="#06b6d4" radius={[6, 6, 0, 0]} />
                        <Bar dataKey="gate_sorties_pleins" name="Pleins sortants" fill="#f59e0b" radius={[6, 6, 0, 0]} />
                        <Bar dataKey="gate_sorties_vides" name="Vides sortants" fill="#8b5cf6" radius={[6, 6, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                <SectionCard title="Navires attendus du jour" subtitle={<><DataBadge type="jour" /> Bulletin du {formatDateLabel(selectedSituationDate)}</>}>
                  <DataTable
                    columns={[
                ...
                <SectionCard title="Navires appareilles du jour" subtitle={<><DataBadge type="jour" /> Bulletin du {formatDateLabel(selectedSituationDate)}</>}>
                  <DataTable
                    columns={[
                      { key: "nom_navire", label: "Navire" },
                      { key: "shipping", label: "Ligne", render: (r) => <ShippingBadge rawValue={r.shipping} /> },
          ...
          {activeTab === "cumul2026" && (
            <>
              {/* KPI cards */}
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
                <MetricCard label={`Cumul TEU ${selectedCumulYear}`} value={formatInteger(cumul2026Annual.totalTeu)} tone="#10b981" hint={`Budget ${formatInteger(cumul2026Annual.totalForecast)} | Real. ${formatPercent(cumul2026Annual.tauxRealisationLast)}`} icon={<Activity className="h-4 w-4" />} compact />
                <MetricCard label="Import cumule" value={formatInteger(cumul2026Annual.importTeu)} tone="#3b82f6" hint={`${cumul2026Monthly.length ? formatPercent(cumul2026Annual.importTeu / cumul2026Annual.totalTeu * 100) : "—"} du total`} icon={<ArrowRightLeft className="h-4 w-4" />} compact />
                <MetricCard label="Export cumule" value={formatInteger(cumul2026Annual.exportTeu)} tone="#06b6d4" icon={<ArrowRightLeft className="h-4 w-4" />} compact />
                <MetricCard label="Transbo cumule" value={formatInteger(cumul2026Annual.transboTeu)} tone="#f59e0b" icon={<Container className="h-4 w-4" />} compact />
                <MetricCard label="Occupation moy." value={`${cumul2026Annual.occupationAvg.toFixed(0)}%`} tone="#8b5cf6" hint={`Reefers moy. ${cumul2026Annual.reefersAvg.toFixed(0)}%`} icon={<Container className="h-4 w-4" />} compact />
                <MetricCard label="TTT moy. / jour" value={formatMinutes(cumul2026Annual.tttAvg)} tone="#0f766e" hint={`${cumul2026Annual.camionsAvgJour.toFixed(0)} camions/j | Prod. ${cumul2026Annual.productivityAverage.toFixed(1)}`} icon={<Gauge className="h-4 w-4" />} compact />
              </div>

              {/* Cumul TEU par mois (barres) + Moyennes sorties/jour par mois */}
              <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
                <SectionCard title="Cumul TEU par mois" subtitle={<><DataBadge type="cumul-mois" /> Realisation au dernier jour de chaque mois</>}>
                  <div className="h-[380px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={cumul2026Monthly}>
                        ...
                        <Tooltip content={<ChartTooltip valueFormatter={(v) => formatInteger(v)} />} />
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Bar dataKey="importTeu" name="Import" fill="#3b82f6" radius={[4, 4, 0, 0]} stackId="teu" />
                        <Bar dataKey="exportTeu" name="Export" fill="#10b981" radius={[0, 0, 0, 0]} stackId="teu" />
                        <Bar dataKey="transboTeu" name="Transbo" fill="#f59e0b" radius={[0, 0, 0, 0]} stackId="teu" />
                        <Bar dataKey="videsTeu" name="Vides" fill="#8b5cf6" radius={[4, 4, 0, 0]} stackId="teu" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>

                <SectionCard title="Moyennes journalieres" subtitle={<><DataBadge type="moy-annuel" /> Moyenne annuelle {selectedCumulYear}</>}>
                  <DataTable
                    columns={[
                      { key: "name", label: "Indicateur" },
                      { key: "value", label: "Moy. / jour", align: "right" },
                    ]}
                    rows={[
                      { name: "Sorties pleins", value: cumul2026Annual.sortiesPleinAvgJour.toFixed(0) },
                      { name: "Sorties vides", value: cumul2026Annual.sortiesVideAvgJour.toFixed(0) },
                      { name: "Total sorties", value: cumul2026Annual.sortiesTotalAvgJour.toFixed(0) },
                      { name: "Camions total", value: cumul2026Annual.camionsAvgJour.toFixed(0) },
                      ...
                      { name: "Productivite", value: cumul2026Annual.productivityAverage.toFixed(1) },
                    ]}
                    compact
                  />
                </SectionCard>
              ...
              {/* Moyenne sorties camions/jour par mois + TTT moyen/jour par mois */}
              <div className="grid gap-5 xl:grid-cols-2">
                <SectionCard title="Moy. sorties camions / jour par mois" subtitle={<><DataBadge type="moy-mois" /> Moyenne journaliere par type de sortie</>}>
                  <div className="h-[340px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={cumul2026Monthly}>
                        <CartesianGrid {...CHART_GRID_PROPS} />
                        ...
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Bar dataKey="sortiesPleinAvgJour" name="Sorties pleins/j" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="sortiesVideAvgJour" name="Sorties vides/j" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="camionsAvgJour" name="Camions total/j" fill="#06b6d4" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>
                  ...
                  <div className="h-[340px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={cumul2026Monthly}>
                        ...
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Line type="monotone" dataKey="tttAvg" name="TTT moy./j" stroke="#f59e0b" strokeWidth={2.5} dot={{ r: 4 }} />
                        <Line type="monotone" dataKey="productivityAverage" name="Prod. moy." stroke="#10b981" strokeWidth={2.5} dot={{ r: 4 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>
                  ...
                  <div className="h-[340px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={cumul2026Monthly}>
                        ...
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Area type="monotone" dataKey="occupationAvg" name="Parc %" stroke="#8b5cf6" fill="url(#gradOcc)" strokeWidth={2.5} />
                        <Area type="monotone" dataKey="reefersAvg" name="Reefers %" stroke="#06b6d4" fill="url(#gradReef)" strokeWidth={2.5} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>
              ...
              <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
                <SectionCard title="Flux gate cumules" subtitle={<><DataBadge type="cumul-annuel" /> Cumul {selectedCumulYear}</>}>
                  <DataTable
                    ...
                    rows={[
                      { name: "Entrees pleins", cumul: formatInteger(cumul2026Annual.gateEntreesPleins), avgJour: (cumul2026Annual.gateEntreesPleins / Math.max(cumul2026Monthly.reduce((s, r) => s + r.nbJours, 0), 1)).toFixed(0) },
                      { name: "Entrees vides", cumul: formatInteger(cumul2026Annual.gateEntreesVides), avgJour: (cumul2026Annual.gateEntreesVides / Math.max(cumul2026Monthly.reduce((s, r) => s + r.nbJours, 0), 1)).toFixed(0) },
                      { name: "Sorties pleins", cumul: formatInteger(cumul2026Annual.gateSortiesPleins), avgJour: (cumul2026Annual.gateSortiesPleins / Math.max(cumul2026Monthly.reduce((s, r) => s + r.nbJours, 0), 1)).toFixed(0) },
                      { name: "Sorties vides", cumul: formatInteger(cumul2026Annual.gateSortiesVides), avgJour: (cumul2026Annual.gateSortiesVides / Math.max(cumul2026Monthly.reduce((s, r) => s + r.nbJours, 0), 1)).toFixed(0) },
          ...
          {activeTab === "operations" && (
            <>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <MetricCard label="Navires en op." value={formatInteger(latestDaily.nb_navires_en_operation)} tone="#3b82f6" hint={`${formatInteger(latestDaily.nb_navires_attendus)} att. | ${formatInteger(latestDaily.nb_navires_appareilles)} app.`} icon={<Ship className="h-4 w-4" />} compact />
                <MetricCard label="Reefers" value={formatPercent(latestDaily.taux_occupation_reefers)} tone="#8b5cf6" hint={`${formatInteger(latestDaily.reefers_utilises)} / ${formatInteger(latestDaily.reefers_total)} plugs`} icon={<Container className="h-4 w-4" />} compact />
                <MetricCard label="Utilisation" value={formatPercent(latestDaily.kpi_utilisation_globale_pct)} tone="#f59e0b" hint={`${toNumber(latestDaily.kpi_net_prod_moy_appareilles).toFixed(1)} mvts/h`} icon={<Gauge className="h-4 w-4" />} compact />
              </div>
              ...
              <div className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
                  ...
                  <div className="h-[360px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={dashboardData.dailyData}>
                        ...
                        <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} valueFormatter={(v) => formatInteger(v)} />} />
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Area type="monotone" dataKey="total_teu" name="Realise" stroke="#10b981" fill="url(#gradReal)" strokeWidth={2.5} />
                        <Area type="monotone" dataKey="total_forecast" name="Budget" stroke="#3b82f6" fill="url(#gradBudget)" strokeWidth={2} strokeDasharray="6 3" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>
                ...
                >
                  <div className="h-[360px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={flowMix} dataKey="value" nameKey="name" innerRadius={75} outerRadius={120} paddingAngle={3} cornerRadius={4}>
                  ...
                  <div className="h-[320px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={[{
                        ...
                        <Tooltip content={<ChartTooltip valueFormatter={(v) => formatInteger(v)} />} />
                        <Legend wrapperStyle={{ fontSize: 12 }} />
                        <Bar dataKey="Pleins entrant" fill="#3b82f6" radius={[6, 6, 0, 0]} />
                        <Bar dataKey="Vides entrant" fill="#06b6d4" radius={[6, 6, 0, 0]} />
                        <Bar dataKey="Pleins sortant" fill="#f59e0b" radius={[6, 6, 0, 0]} />
                        <Bar dataKey="Vides sortant" fill="#8b5cf6" radius={[6, 6, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </SectionCard>
              ...
              <div className="flex items-center gap-4">
                  ...
                  >
                    {monthlyBulletin.slice().reverse().map((b) => (
                      <option key={b.anneeMois} value={b.anneeMois}>
                        {b.moisLabel} {b.annee} — {formatDateLabel(b.latestDate)}
                      </option>
                    ))}
                  </select>
                </div>
                {activeBulletin && (
                  <span className="text-[13px] text-[var(--text-secondary)]">
                    Derniere mise a jour : <strong>{formatDateLabel(activeBulletin.latestDate)}</strong>
                  </span>
                )}
              </div>
            ...
            <>
              ...
              {activeSegment === "volumes" && (
                <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
                  <SectionCard title="Volumes TEU" subtitle="Evolution import, export, transbo, vides">
                    <div className="h-[380px] min-w-0">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={dashboardData.dailyData}>
                          <CartesianGrid {...CHART_GRID_PROPS} />
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.dailyData.slice(-12).reverse()}
                  />
                </div>
              )}
              ...
              {activeSegment === "gate" && (
                <div className="space-y-5">
                  <div className="grid gap-5 xl:grid-cols-2">
                    <SectionCard title="Gate / TTT" subtitle="Camions et mouvements par date">
                      <div className="h-[380px] min-w-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={dashboardData.gateData.slice(-12)}>
                            <CartesianGrid {...CHART_GRID_PROPS} />
                            <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                            <YAxis {...CHART_AXIS_PROPS} />
                            <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} valueFormatter={(v) => formatInteger(v)} />} />
                    ...
                    <SectionCard title="Evolution du TTT" subtitle="Dernieres fiches gate disponibles">
                      <div className="h-[380px] min-w-0">
                        <ResponsiveContainer width="100%" height="100%">
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.gateData.slice(-12).reverse()}
                  />
                </div>
              )}
              ...
              {activeSegment === "escales" && (
                <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
                  <SectionCard title="Escales armateurs" subtitle="Prevues vs realisees">
                    <div className="h-[380px] min-w-0">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={dashboardData.armateursData.slice(-12)}>
                          <CartesianGrid {...CHART_GRID_PROPS} />
                          <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                          <YAxis {...CHART_AXIS_PROPS} />
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.armateursData.slice(-12).reverse()}
                  />
                </div>
              )}
              ...
              {activeSegment === "exploitants" && (
                <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
                  <SectionCard title="Exploitants parc" subtitle="Stocks par exploitant">
                    <div className="h-[380px] min-w-0">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={dashboardData.exploitantsData.slice(-12)}>
                          <CartesianGrid {...CHART_GRID_PROPS} />
                          <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                          <YAxis {...CHART_AXIS_PROPS} />
                          <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} valueFormatter={(v) => formatInteger(v)} />} />
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.exploitantsData.slice(-12).reverse()}
                  />
                </div>
              )}
              ...
              {activeSegment === "kpis" && (
                <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
                  <SectionCard title="KPIs" subtitle="Productivite et utilisation">
                    <div className="h-[380px] min-w-0">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={dashboardData.kpisData.slice(-12)}>
                          <CartesianGrid {...CHART_GRID_PROPS} />
                          <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                          <YAxis {...CHART_AXIS_PROPS} />
                          <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} />} />
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.kpisData.slice(-12).reverse()}
                  />
                </div>
              )}
              ...
              {activeSegment === "parc" && (
                <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
                  <SectionCard title="Parc conteneurs" subtitle="Utilise, disponible et taux">
                    <div className="h-[380px] min-w-0">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={dashboardData.parcConteneurs.slice(-12)}>
                          <CartesianGrid {...CHART_GRID_PROPS} />
                          <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                          <YAxis {...CHART_AXIS_PROPS} />
                          <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} />} />
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.parcConteneurs.slice(-12).reverse()}
                  />
                </div>
              )}
              ...
              {activeSegment === "rapport" && (
                <div className="grid gap-5 xl:grid-cols-[1.2fr_1fr]">
                  <SectionCard title="Rapport quotidien" subtitle="Navires attendus, en operation, appareilles">
                    <div className="h-[380px] min-w-0">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={dashboardData.rapportQuotidien.slice(-12)}>
                          <CartesianGrid {...CHART_GRID_PROPS} />
                          <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                          <YAxis {...CHART_AXIS_PROPS} />
                          <Tooltip content={<ChartTooltip labelFormatter={formatDateLabel} />} />
                  ...
                  <DataTable
                    ...
                    rows={dashboardData.rapportQuotidien.slice(-12).reverse()}
                  />
                </div>
              )}
            </>
          ...
          {activeTab === "navires" && (
            <>
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                ...
                <SectionCard title="Cadence d'execution" subtitle="Camions, TTT et productivite sur la periode">
                  <div className="h-[360px] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={dashboardData.dailyData}>
                        <CartesianGrid {...CHART_GRID_PROPS} />
            ...
            <>
              ...
              {analyseSubTab === "daily" && dailyAnalysis && (
                <>
                  ...
                  <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
                    <SectionCard title="Performance vs forecast" subtitle="Evolution realise vs budget sur la periode">
                      <div className="h-[360px] min-w-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={dashboardData.dailyData}>
                            <defs>
                  ...
                  <div className="grid gap-5 xl:grid-cols-2">
                    <SectionCard title="Taux d'occupation - Tendance 21j" subtitle="Parc et reefers">
                      <div className="h-[300px] min-w-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={dashboardData.parcConteneurs.slice(-21)}>
                            <CartesianGrid {...CHART_GRID_PROPS} />
                            <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                    ...
                    <SectionCard title="Productivite - Tendance 21j" subtitle="Prod. nette et TTT">
                      <div className="h-[300px] min-w-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={dashboardData.gateData.slice(-21)}>
                            <CartesianGrid {...CHART_GRID_PROPS} />
                            <XAxis dataKey="date_rapport" {...CHART_AXIS_PROPS} tickFormatter={formatShortDate} />
                            <YAxis yAxisId="left" {...CHART_AXIS_PROPS} />
                            <YAxis yAxisId="right" orientation="right" {...CHART_AXIS_PROPS} />
